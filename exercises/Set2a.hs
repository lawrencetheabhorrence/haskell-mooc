...
updateAt :: Int -> a -> [a] -> [a]
updateAt i x xs = (take i xs) ++ x1 ++ (drop (i + 1) xs) where x1 = [x]
...
substring i j s = if (i == j) then "" else  x ++ substring (i+1) j s where x = [(s !! i)]
------------------------------------------------------------------------------
...
palindromify s = if(reverse s == s) then s else palindromify(drop 1 (take (length s - 1) s))
