...
joinToLength x strings = filter (\y -> (length y) == x) (allConcatsInStringArray strings)

-- concatenates a string to all strings in an array
concatStringtoStringArray :: String -> [String] -> [String]
concatStringtoStringArray x strings = map (x ++) strings

concatStringtoStringArrayreverse :: String -> [String] -> [String]
concatStringtoStringArrayreverse x strings = map (++ x) strings

--generates an array with all possible concat combinations
--okay i think that was a really bad explanation
--just take a look okay

allConcatsInStringArray :: [String] -> [String]
allConcatsInStringArray [] = []
allConcatsInStringArray (x:xs) = (concatStringtoStringArray x (x:xs)) ++ (concatStringtoStringArrayreverse x (x:xs)) ++ (allConcatsInStringArray xs)
...
sumRights x = sumInts (dropLefts x)

dropLefts :: [Either a Int] -> [Int]
dropLefts x = map (either (\y -> 0) id) x
