...
allEqual [] = True
allEqual [x] = True
allEqual (x:xs:xss) = if(x /= xs) then False else allEqual (xs:xss)
...
distinct xs = if (length (xs \\ (nub xs)) > 0) then False else True
...
longest :: (Ord a) => [[a]] -> [a]
longest xs = minimumBy (comparing head) (filter (\x -> (length x) == length (maximumBy (comparing length) xs)) xs)
...
incrementKey k [] = []
incrementKey k (v:vs) = if ((fst v) == k) then [(k, ((snd v) + 1))] ++ (incrementKey k vs) else [v] ++ (incrementKey k vs)
...
freqs xs = Map.fromList (freqs' 1 (sort xs))

freqs' :: (Eq a, Ord a) => Int -> [a] -> [(a, Int)]
freqs' _ [] = []
freqs' 1 [x] = [(x, 1)]
freqs' 1 [x, xs] =  if(x == xs) then [(x, 2)] else [(x, 1), (xs, 1)]
freqs' n [x] = [(x, (n + 1))]
freqs' n (x:xs:xss)
    | (x == xs) = if(xss == []) then [(x, n + 1)] else freqs' (n + 1) (xs:xss)
    | otherwise = [(x, n)] ++ (freqs' 1 (xs:xss))
...
transfer from to amount bank = if((Map.notMember from bank) || (Map.notMember to bank) || amount < 0 || (Map.lookup from bank) < Just amount) then bank else Map.adjust (\y -> y - amount) from (Map.adjust (amount +) to bank)
...
maxIndex xs = fst . head . reverse $ (sortBy (comparing snd) (assocs xs))
